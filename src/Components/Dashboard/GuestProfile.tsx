...
export default function GuestProfile() {
  ...
  useEffect(() => {
    const fetchAttendedBookings = async () => {
      ...
      const attendedBookings = booking.items.filter(
        (item: Booking) => item.guestTicket?.redeemed === true
      );
                ...
                {attendedBookings && (
                  ...
                  >
                    <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                      ...
                      >
                        {attendedBookings.map((event) => (
                          <Avatar
                            key={event.bookingGuestId}
                            src={`${dbStorage}${event.event.image}`}
                          />
                        ))}
                      </AvatarGroup>
                      <Typography
