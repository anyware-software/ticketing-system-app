...
const GRAPHQL_API_KEY =
  ...
  const { createTransaction: createTransactionQuery , updateBooking:updateBookingQuery } = require("../constants/queries");
...
const updateBooking = async ( eventBookingID, bookAttributes ) => { 
    variables = {
        input: {
          id: eventBookingID,
...
const handleSuccessPayment = async (params) => { 
    const { paymentObj , eventBookingID , bookAttributes} = params;
    // update Booking
    // create a transaction
    const [transaction] = await Promise.all([createTransaction(paymentObj),updateBooking(eventBookingID , bookAttributes)]);
